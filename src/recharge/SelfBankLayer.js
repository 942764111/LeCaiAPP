 ...
 */
var G_SELFBANKLAYER;
var SelfBankLayer = cc.Layer.extend({
    _mobileNumBox:null,
    _passwordBox:null,
    ...
    ctor:function () {
        ...
        this.addChild(line1);
        this._mobileNumBox = new cc.EditBox(cc.size(380, 50), new cc.Scale9Sprite(res.png_bg_input));
        this._mobileNumBox.x = GC.W / 2+100;
        ...
        // this._mobileNumBox.setInputMode(cc.EDITBOX_INPUT_MODE_SINGLELINE);
        this._mobileNumBox.setPlaceHolder("XXX");
        ...
        this._zhongguoBank.y = this._mobileNumBox.y - this._mobileNumBox.height - 65;
        ...
        bankStr.y = this._mobileNumBox.y - this._mobileNumBox.height - 65;
        ...
        zhihangStr.y = this._passwordBox.y - this._passwordBox.height - 60;
        ...
        yesBtn.addClickEventListener(function () {
            cc.audioEngine.playEffect(res.mp3_click);
            if(G_SELFBANKLAYER._passwordBox.getString() != "" &&
                G_SELFBANKLAYER._zhongguoBank.getTitleText() != "" &&
                G_SELFBANKLAYER._mobileNumBox.getString() != "" &&
                G_SELFBANKLAYER._passwordConfirmBox.getString() != ""
            )
            {
                var reqUrl = GC.URL + "user/addBankCard?"
                    + "account_number="+G_SELFBANKLAYER._passwordBox.getString()
                    + "&bank_name="+G_SELFBANKLAYER._zhongguoBank.getTitleText()
                    + "&real_name="+G_SELFBANKLAYER._mobileNumBox.getString()
                    + "&branch_bank="+G_SELFBANKLAYER._passwordConfirmBox.getString()
                    + "&client_id="+GC.CLIENT_ID
                    + "&token="+GC.TOKEN;
                th.Http.inst().get(encodeURI(reqUrl),function (isSuccess, data) {
                    if(isSuccess && data.code == "200")
                    {
                        var chenggong=new th.PopupLayer("银行卡添加成功","确定",function(){
                            G_SELFBANKLAYER.close();
                        ...
                        G_SELFBANKLAYER.addChild(chenggong);
                    }else
                    {
                        var chenggong=new th.PopupLayer(data.message,"确定",function(){
                        });
        ...
        noBtn.addClickEventListener(function () {
            cc.audioEngine.playEffect(res.mp3_click);
            G_SELFBANKLAYER._mobileNumBox.setString("");
            G_SELFBANKLAYER._passwordBox.setString("");
            G_SELFBANKLAYER._passwordConfirmBox.setString("");
        });
        this.addChild(noBtn);
    ...
    tableCellTouched:function (table, cell) {
        G_SELFBANKLAYER._zhongguoBank.setTitleText(cell.getChildByTag(1).getName());
        G_SELFBANKLAYER._shijianBg.visible = !G_SELFBANKLAYER._shijianBg.visible;
    ...
    tableCellAtIndex:function (table, idx) {
        ...
        if (!cell) {
            ...
            bianhao = new cc.LabelTTF(G_SELFBANKLAYER._bankList[G_SELFBANKLAYER._bankList.length - 1- idx]+"","微软雅黑", 35);
            bianhao.setTag(1);
            bianhao.setColor(cc.color(236,64,28,0));
            bianhao.setPosition(cc.p(180,25));
            bianhao.setName(G_SELFBANKLAYER._bankList[G_SELFBANKLAYER._bankList.length - 1-idx]+"");
        ...
        }else
        {
            ...
            bianhao.setString(G_SELFBANKLAYER._bankList[G_SELFBANKLAYER._bankList.length - 1-idx]+"");
            bianhao.setName(G_SELFBANKLAYER._bankList[G_SELFBANKLAYER._bankList.length - 1-idx]+"");
        }
        return cell;
    },
    numberOfCellsInTableView:function (table) {
        return G_SELFBANKLAYER._bankList.length;
    },
    editBoxEditingDidEnd: function (editBox) {
        switch(editBox.getName())
        {
            case"mobileNumBox":
                ...
                {
                    var alertStr = new th.AlertLayer(G_SELFBANKLAYER,res.png_img_tishikuang,"请填写真确姓名");
                    // G_SELFBANKLAYER._mobileNumBox.setString("");
                }
                break;
            case"passwordBox":
                ...
                {
                    var alertStr = new th.AlertLayer(G_SELFBANKLAYER,res.png_img_tishikuang,"请填写正确卡号");
                    // G_SELFBANKLAYER._passwordBox.setString("");
                }
                break;
            case"passwordConfirmBox":
                ...
                {
                    var alertStr = new th.AlertLayer(G_SELFBANKLAYER,res.png_img_tishikuang,"请填写正确的开户支行");
                    // G_SELFBANKLAYER._passwordConfirmBox.setString("");
                }
                break;
        }
