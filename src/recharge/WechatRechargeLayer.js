 ...
 * 微信充值
 */
var WECHATRECHARGELAYER;
var WechatRechargeLayer = cc.Layer.extend({
    _jine:null,
    _msg:null,
    ctor: function (msg,btnText) {
        ...
        yesBtn.addClickEventListener(function () {
            ...
            for(var i = 0; i < arr.length ; i++)
            {
                ...
                {
                    case "recharge_cash":
                        arr[i] = ("recharge_cash="+WECHATRECHARGELAYER._jine.getString());
                        str += arr[i];
                        break;
                    case "pay_type":
                        arr[i] = ("pay_type=wechat");
                        str += arr[i];
                        break;
                    case "nonce_str":
            ...
            str += "guangcai";
            sign = hex_md5(str);
            if(WECHATRECHARGELAYER._jine.getString() != "")
            {
                var reqUrl = GC.URL + "pay/onlineRecharge?"
                    + "recharge_cash="+WECHATRECHARGELAYER._jine.getString()
                    + "&pay_type=wechat"
                    + "&nonce_str="+nonce_str
                    + "&sign="+sign
            ...
            {
                var alertStr = new th.AlertLayer(WECHATRECHARGELAYER,res.png_img_tishikuang,"请输入正确金额");
                WECHATRECHARGELAYER._jine.setString("");
            }
        });
    ...
    editBoxEditingDidEnd: function (editBox) {
        switch(editBox.getName())
        {
            case"jine":
                var patrn=/^[0-9]+$/;
                if(patrn.test(editBox.getString()) && Number(editBox.getString()) >= WECHATRECHARGELAYER._msg.minRecharge && Number(editBox.getString())<= WECHATRECHARGELAYER._msg.maxRecharge)
                {
                }else
                {
                    var alertStr = new th.AlertLayer(WECHATRECHARGELAYER,res.png_img_tishikuang,"请输入正确金额");
